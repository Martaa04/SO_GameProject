...
void PlayerGames(char output[512], MYSQL *conn)
{
	...
	strcpy(output, "1/"); // Reset output buffer
...
void LogIn( Connected *list, char username[80],char password[80], char output[80], MYSQL *conn, int sock_conn)
{
	...
	{	
		...
		if (strcmp(password, password_database) == 0) {
			//añadir lista de conectados detrás del 6/3
			sprintf(output, "6/%d", 3);  //Successful login
	...
	while (terminar==0){
		...
		char username[80], password[80], email[80];
			...
			case 2:
				...
				strcpy(password,p);
				printf("FFFF");
				LogIn(&Connlist,username, password, output, conn, sock_conn);
				if (output==3){
			...
			case 4:
				...
				break;
			case 5:
				GiveMeConnected(&Connlist, output);  // Agrega los jugadores conectados a la notificaciÃ
				break;
